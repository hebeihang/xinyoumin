    ...
    public function SetSource( $source, $source_charset='utf-8', $target_charset='utf-8' )
    {
        ...
        if( $source != '' )
        {
            ...
            if( preg_match("/^utf/", $source_charset) ) {
                ...
                $this->sourceString = mb_convert_encoding(mb_convert_encoding( $source,'utf-8','gb18030'), UCS2,'utf-8');
            }
            else if( preg_match("/^big/", $source_charset) ) {
                //$this->sourceString = iconv('utf-8', UCS2, iconv('big5', 'utf-8', $source));
                $this->sourceString = mb_convert_encoding(mb_convert_encoding($source,'big5','utf-8'),UCS2,'utf-8');
            }
            else {
                $rs = false;
            }
    ...
    public function LoadDict( $maindic='' )
    {
        ...
        foreach($ds as $d)
        {
            ...
            else
            {
                ...
                $spstr = mb_convert_encoding($spstr,"utf-8",UCS2);
                $ws = explode(',', $d);
                //$wall = iconv('utf-8', UCS2, join($spstr, $ws));
                $wall = mb_convert_encoding(join($spstr, $ws), UCS2, 'utf-8');
            ...
            if($cn < 0x80)
            {
                ...
                else
                {
                    if( $onstr != '' )
                    {
                        ...
                        if( $lastc==2 )
                        {
                            if( !preg_match('/'.$notNumberMatch.'/i', mb_convert_encoding($onstr, 'utf-8', UCS2)) ) $lastc = 4;
                        }
                        $this->simpleResult[$s]['t'] = $lastc;
     ...
     */
     private function _out_string_encoding( &$str )
     {
        ...
        if( $rsc==1 ) {
            ...
            $rsstr = mb_convert_encoding($str, 'utf-8',UCS2);
        }
        else if( $rsc==2 ) {
            //$rsstr = iconv('utf-8', 'gb18030', iconv(UCS2, 'utf-8', $str) );
            $rsstr = mb_convert_encoding(mb_convert_encoding($str,'utf-8',UCS2),'gb18030','utf-8');
        }
        else{
            //$rsstr = iconv('utf-8', 'big5', iconv(UCS2, 'utf-8', $str) );
            $rsstr = mb_convert_encoding(mb_convert_encoding(  $str,'utf-8',UCS2), 'big5' ,'utf-8');
        }
        return $rsstr;
     }
     ...
     public function MakeDict( $source_file, $target_file='' )
     {
        ...
        while( $line = fgets($fp, 512) )
        {
           ...
           // $w = iconv('utf-8', UCS2, $w);
            $w = mb_convert_encoding($w,UCS2,'utf-8');
            $k = $this->_get_index( $w );
            if( isset($allk[ $k ]) )
                $allk[ $k ][ $w ] = array($r, $a);
            else
     ...
     public function ExportDict( $targetfile )
     {
        ...
        for($i=0; $i <= $this->mask_value; $i++)
        {
            ...
            foreach($data as $k => $v)
            {
                ...
                $w =  mb_convert_encoding($k,'utf-8',UCS2);
                fwrite($fp, "{$w},{$v[0]},{$v[1]}\n");
            }
        }
        fclose( $fp );
