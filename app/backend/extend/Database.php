...
class Database extends Backend
{
    ...
    public function __construct(App $app)
    {
        ...
        $this->config=array(
            'path'     => root_path('backup').'database'.DS, // 数据库备份路径
    ...
    public function database()
    {
        ...
        // 提示信息
        $pageTips = '数据库中共有 ' . count($list) . ' 张表，共计 ' . formatBytes($total).'<br>注意：若数据库较大建议使用客户端或命令行方式进行备份';
        ...
        if ($this->request->param('_list'))
        {
            ...
            if (Request::param('name')) {
                foreach ($list as $k => $v) {
                    if (!strpos($v['name'], Request::param('name'))) {
                        unset($list[$k]);
                    }
                }
            }
        ...
        return $this->tableBuilder
            ...
            ->addTopButtons([
                'backup' => [
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => 'AWS_ADMIN.operate.database(\''. (string)url('backup').'\' , \'备份\')',
                ],
                'optimize' => [
                    'title'       => '优化',
                    'icon'        => 'fa fa-medkit',
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => 'AWS_ADMIN.operate.database(\''. (string)url('optimize').'\', \'优化\')',
                ],
                'repair' => [
                    'title'       => '修复',
                    'icon'        => 'fa fa-user-md',
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => 'AWS_ADMIN.operate.database(\''. (string)url('repair').'\', \'修复\')',
                ],
            ])
            ->fetch();
    }
    ...
    public function restore()
    {
        ...
        foreach ($list as $k => $v) {
            $total += $v['size'];
            $list[$k]['size'] = formatBytes($v['size']);
            $listNew[] = $list[$k];
        }
        $list = $listNew;
        ...
        // 提示信息
        $pageTips = '备份文件列表中共有 ' . count($list) . ' 个文件，共计 ' . formatBytes($total).'<br>注意：若备份文件有多个卷，请务必多个卷都要还原';
